...
import "../../src/app/styles/page/_AdminPanel.scss";
...
const AdminPanel = () => {
  ...
  const fetchAllUsersData = async () => {
    try {
      const employeesCollection = collection(db, "employeeDetails");
      ...
      // Sort alphabetically by personalInfo.name
      users.sort((a, b) =>
  ...
  const handleSearch = (e) => {
    ...
    const filtered = usersData.filter((user) =>
      user.personalInfo?.name?.toLowerCase().includes(query)
    );
  ...
  const updateDepartment = async (userId, department) => {
    ...
    try {
      const userRef = doc(db, "employeeDetails", userId);
      await setDoc(
        userRef,
  ...
  return (
    <>
      ...
      <main className="maincontainer">
              ...
              <tbody>
                {filteredUsers.map((user, index) => (
                  <tr key={user.uid}>
                    ...
                    <td>
                      <Link
                        href={`/admin/users-session/${user.uid}`}
                        className="user-link"
                      >
                    ...
                    <td>
                      <div className="search">
                        <select
                          ...
                          onChange={(e) =>
                            updateDepartment(user.uid, e.target.value)
                          }
                        >
        ...
        >
          <h2>Send Message to {selectedUser?.personalInfo?.name}</h2>
          <div className="form-group">
            ...
            <div className="btn-container">
              <button onClick={sendNotification} className="approve-btn">
                Send
              </button>
