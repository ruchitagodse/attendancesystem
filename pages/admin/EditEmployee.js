...
export default function EditEmployee({ employee, onClose, onUpdate }) {
    ...
    try {
      ...
      for (let key of Object.keys(kycDetails)) {
        if (kycDetails[key] instanceof File) {
          ...
          let folder = key;
          if (["experienceLetters", "payslips", "certifications", "relievingLetters"].includes(key)) continue; // handled below
          uploadedFiles[key] = await uploadFile(kycDetails[key], folder);
        }
      }
  ...
  return (
    <div className="edit-employee-modal">
      <h2>Edit Employee: {personalInfo.name}</h2>
      ...
      {activeTab === "personal" && (
        <div className="form-grid">
          <label>Name:<input type="text" name="name" value={personalInfo.name || ""} onChange={handlePersonalChange} /></label>
          <label>Email:<input type="email" name="email" value={personalInfo.email || ""} onChange={handlePersonalChange} /></label>
          <label>Mobile:<input type="text" name="mobile" value={personalInfo.mobile || ""} onChange={handlePersonalChange} /></label>
          <label>Alt Mobile:<input type="text" name="altMobile" value={personalInfo.altMobile || ""} onChange={handlePersonalChange} /></label>
          <label>Date of Birth:<input type="date" name="dob" value={personalInfo.dob || ""} onChange={handlePersonalChange} /></label>
          <label>Gender:
          ...
          <label>Company:<input type="text" name="company" value={personalInfo.company || ""} onChange={handlePersonalChange} /></label>
          <label>Department:<input type="text" name="department" value={personalInfo.department || ""} onChange={handlePersonalChange} /></label>
        ...
        <div className="form-grid">
          {["aadhaarFile", "panFile", "passportFile", "voterIdFile", "profilePhoto"].map(key => (
            <label key={key}>
              ...
              {kycDetails[key] && typeof kycDetails[key] === "string" ? (
                <p>Existing: <a href={kycDetails[key]} target="_blank">{kycDetails[key].split("/").pop()}</a></p>
              ) : kycDetails[key] instanceof File ? <p>New: {kycDetails[key].name}</p> : null}
            </label>
          ))}
          ...
          {["experienceLetters", "payslips", "certifications", "relievingLetters"].map(key => (
            <label key={key}>
              ...
              {kycDetails[key] && Array.isArray(kycDetails[key]) && kycDetails[key].map((f, i) => (
                <p key={i}>{typeof f === "string" ? <a href={f} target="_blank">{f.split("/").pop()}</a> : f.name}</p>
              ))}
            </label>
          ))}

          <label>Bank Details / Cancelled Cheque:<textarea name="bankDetails" value={kycDetails.bankDetails || ""} onChange={handleKycChange}></textarea></label>
          <label>Medical Certificate:<textarea name="medicalCertificateText" value={kycDetails.medicalCertificateText || ""} onChange={handleKycChange}></textarea></label>
        </div>
      )}
      ...
      {activeTab === "hr" && (
        <div className="form-grid">
          <label>Probation Period:<input type="date" name="probationPeriod" value={hrDetails.probationPeriod || ""} onChange={handleHrChange} /></label>
          ...
          <label>Reporting Manager:<input type="text" name="reportingManager" value={hrDetails.reportingManager || ""} onChange={handleHrChange} /></label>
      ...
      <div className="actions">
        <button onClick={handleSubmit} disabled={loading}>{loading ? "Updating..." : "Update Employee"}</button>
